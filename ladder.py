...
def addmontgomery(p,q,A,B,prime):
    """
    (a) If P=OP=O, then P+Q=QP+Q=Q.
    (b) Otherwise, if Q=OQ=O, then P+Q=PP+Q=P.
    (c) Otherwise, write P=(x1,y1)P=(x1,y1) and Q=(x2,y2)Q=(x2,y2).
    ...
    diff1 = (q[1] - p[1]) % prime
    diff2 = (q[0] - p[0]) % prime
    diffinv = pow(diff2,-1,prime)
    alpha = diff1*diffinv % prime
    x3 = (B*alpha*alpha - p[0] - q[0] - A) % prime
    y3 = (alpha*(p[0] - x3) - p[1]) % prime
    return (x3,y3)

    
def doublemontgomery(p,A,B,prime):
    """
    (a) If P=OP=O, then 2P=2P=O.
    (b) Otherwise, write P=(x1,y1)P=(x1,y1).
    """
    if p[0] is None:
        return (None,None)
    alphadenominator = 2*B*p[1]
    alphainv = pow(alphadenominator,-1,prime)
    alpha = (3*p[0]*p[0] + 2*A*p[0] +1)*alphainv % prime
    x3 = (B*alpha*alpha - A- 2*p[0]) % prime
    y3 = (alpha*(p[0] - x3) - p[1]) % prime    
    return (x3,y3)



def multmontgomery(p,n, a, b, prime):
    if(n <= 0):
        return (None,None) 
    nBits = math.ceil(math.log(n,2))   
    r = (p, addpoint(p,p,a,b,prime)) #  Set (R0,R1) to (P,[2]P)
    for i in range(nBits-2, -1, -1):
        if (n >> i) & 1:
            r = (addmontgomery(r[0],r[1],a,b,prime), doublemontgomery(r[1],a,b,prime))
        else:
            r = (doublemontgomery(r[0],a,b,prime), addmontgomery(r[0],r[1],a,b,prime))        
    return (r[0][0]%prime,r[0][1]%prime)

curveA = 486662
curveB = 1
Gx = 9
Square = pow(Gx,3)+486662*pow(Gx,2)+Gx 
prime = pow(2,255)-19
if legendre_symbol(Square, prime) == 1:
    Gy = tonelli_shanks(Square, prime)
else:
    Gy = None
    exit(1)
G = (Gx,Gy)
n = 0x1337c0decafe
print(n)
Q = multmontgomery(G,n,curveA,curveB,prime)
print(Q)
